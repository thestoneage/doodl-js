...
function frLayout (graph, dimension) {
  ...
  function positions (graph, dimension) {
    graph.each_node(function (node) {
        ...
        dx = locations[node].x + disp[node].x / distance * Math.min(temperature, Math.abs(disp[node].x));
        dy = locations[node].y + disp[node].y / distance * Math.min(temperature, Math.abs(disp[node].y));
        locations[node] = bounce(dimension, new Vector(dx, dy));
        });
  }
