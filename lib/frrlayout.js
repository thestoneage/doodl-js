...
function frLayout (graph, dimension, callback) {
    ...
    function step (graph, dimension){
        ...
        walls(graph, dimension);
        positions(graph, dimension);
        cool();
    }

    function walls(graph, dimension) {
        graph.each_node(function (node) {
           var loc = locations[node];
           calc(loc.sub(new Vector(0, loc.y)), node);
           calc(loc.sub(new Vector(dimension.x, loc.y)), node);
           calc(loc.sub(new Vector(loc.x, 0)), node);
           calc(loc.sub(new Vector(loc.x, dimension.y)), node);
        });

        function calc(delta, node) {
           var distance = Math.max(delta.len(), 0.0001),
               force = Math.pow(force_constant, 2) / distance;
               disp[node] = disp[node].add(delta.mul(force / distance));
        }
    }
    ...
    function positions (graph, dimension) {
        graph.each_node(function (node) {
            ...
            locations[node] = locations[node].add(disp[node].mul(offset / distance));
        });
    }
